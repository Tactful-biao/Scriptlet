                    ...
                    if ii.group() is 'D':
                        ...
                        if ans.upper() == 'Q':
                            if len(wrong) + len(right) == 0:
                                ...
                                print('你总共回答了 %s 道题目! 回答正确了 %s 道， 回答错误了 %s 道！ 正确率为 %s %%'
                                      % (all_subject, len(right), len(wrong), right_rate))
                                if all_subject > 15:
                                    if right_rate > 85:
                                        print('拿高分没问题，正确率超过百分之八十，很厉害哦！')
                                    elif right_rate > 70:
                                        print('正确率超过百分之七十，及格没问题！')
                                    elif right_rate > 60:
                                        print('虽然及格率超过了百分之六十，但是还需要多锻炼！')
                                    else:
                                        print('正确率低于百分之六十，不认真练习的话，可能会挂科哦！')
                                else:
                                    print('答题量少于15道题!请多加练习！')
                                input('按回车键退出！')
                                exit_flag = True
                                break
                        else:
                            if ans.upper() != answer[seed]:
                                if ans.upper() not in 'ABCD' and ans.upper() not in 'ABD' and ans.upper() not in 'ACD':
                                    print('请认真答题, 答案只在ABCD中! 如果不认真答题可能会影响你最终的成绩!')
                                print('回答错误！本题位于第%s章, 正确答案应该是 %s 。\n' % (locate, answer[seed]))
                                wrong_content.append(seed)
                                wrong.append(seed)
                            else:
                                print('哇塞，回答正确，你真棒！ 本题位于第%s章！\n' % locate)
